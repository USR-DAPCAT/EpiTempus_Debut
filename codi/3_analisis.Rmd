...
kk<-dades %>% names() %>% tibble()
colnames(kk)<-"camp"
kk<- kk %>% mutate(dades_generades=1)
pp<-read_excel(conductor)
pp %>% full_join(kk) %>% write.csv2("vars.csv", na="")
...
dades %>% group_by(year_inclusio) %>% count() %>% 
  kable(caption = "Nous pacients DM2 per any") %>% 
