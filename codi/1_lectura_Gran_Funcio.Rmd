...
dt_tabaquisme<-readRDS(here::here(directori_dades_origen,"epiPEUCAT_entregable_tabaquisme_20200206_104913.rds"))
dt_tabaquisme<-dt_tabaquisme %>% transmute(idp,cod="tabac",dat,val) %>% as_tibble()%>%
     semi_join(dt_index,by="idp")

#iii)
dt_diagnostics_AP_HOSP%>% as_tibble()%>%
     semi_join(dt_index,by="idp")

#iv)
dt_facturacio%>% as_tibble()%>% transmute(idp,cod,dat,env)%>%
     semi_join(dt_index,by="idp")
#v)
dt_variables%>%as_tibble()%>%
     semi_join(dt_index,by="idp")
